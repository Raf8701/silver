(function() {
  ...
  function displayCartTotal() {
    ...
    if (prices) {
      ...
      totalPriceContainer.innerHTML = `<span class="Total">Ընդհնուր: ${total.toFixed(
        0
      )} Դ</span>`;
    } else {
      totalPriceContainer.innerHTML = '<span class="total">Total: 0.000Դ</span>';
    }
  }
  ...
  function displayProducts() {
    ...
    if (lsContent !== null) {
      for (let product of lsContent) {
        productMarkup += `
          <tr>
          <td><img class="cart-image" src="${product.image}" alt="Դ{
          product.name
        }" width="120"></td>
          <td>
            ${product.name}
  ...
  function saveProduct(clickedBtn) {
    ...
    lsContent.forEach(function(product) {
      if (product.id === productId) {
        alert("Այս ապրանքը արդեն գտնվում է քարտում");
